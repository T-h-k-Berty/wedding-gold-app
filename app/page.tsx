...
import { motion, AnimatePresence, Variants } from "framer-motion"; // Variants type එක මෙහි එකතු කර ඇත
...
const SectionDivider = () => (
  <motion.div 
    ...
    className="flex items-center justify-center w-full my-12 md:my-24 opacity-80"
  >
...
export default function WeddingInvitation() {
  ...
  // Type Error එක විසඳීම සඳහා Variants type එක යොදා ඇත
  ...
  return (
    <main className="min-h-screen w-full overflow-hidden relative">
      ...
      {step === "website" && (
        <button 
          onClick={toggleAudio}
          className={`fixed bottom-6 right-6 z-50 p-3 md:p-4 rounded-full bg-white/80 backdrop-blur-md shadow-lg border border-yellow-200 text-yellow-700 hover:bg-yellow-50 transition-all ${isPlaying ? 'music-btn-active' : ''}`}
      ...
      <AnimatePresence mode="wait">
        {(step === "envelope" || step === "opening") && (
          ...
          >
            ...
            <div className="relative z-10 flex flex-col items-center px-4 w-full max-w-sm md:max-w-none">
                
                {/* 100% Mobile Responsive Envelope Wrapper */}
                <div 
                  className={`relative cursor-pointer w-full max-w-[280px] sm:max-w-[320px] md:max-w-[384px] h-52 sm:h-60 md:h-72 transition-all duration-1000 ${step === "opening" ? "scale-105" : "hover:scale-105"}`}
                  onClick={handleOpenEnvelope}
                >
                  {/* Envelope Back */}
                  <div className="absolute inset-0 bg-gradient-to-br from-yellow-600 to-yellow-800 rounded-lg shadow-2xl z-0"></div>
                  
                  {/* The Invitation Card (Slides out perfectly) */}
                  <motion.div 
                    className="absolute left-3 right-3 bg-white rounded flex flex-col items-center justify-center text-center shadow-lg border border-gray-200 p-3 sm:p-4 z-[1]"
                    style={{ top: "12px", bottom: "12px" }}
                    initial={{ y: 0 }}
                    animate={step === "opening" ? { y: -120 } : {}}
                    transition={{ duration: 1.2, delay: 0.4, ease: "easeOut" }}
                  >
                    <div className="space-y-1 sm:space-y-2">
                      <p className="text-gray-500 font-serif text-[10px] sm:text-xs md:text-sm tracking-widest uppercase">We are getting married</p>
                      <h1 className="font-playfair text-2xl sm:text-3xl md:text-4xl text-gray-900 font-bold leading-none">{couple.bride_name}</h1>
                      <h1 className="font-playfair text-base sm:text-lg md:text-xl text-yellow-600 italic leading-none">&</h1>
                      <h1 className="font-playfair text-2xl sm:text-3xl md:text-4xl text-gray-900 font-bold leading-none">{couple.groom_name}</h1>
                    </div>
                  </motion.div>
                  ...
                  {step === "envelope" && (
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-30">
                        <button className="bg-white/95 p-3 sm:p-4 rounded-full shadow-[0_10px_25px_rgba(0,0,0,0.5)] text-yellow-600 animate-bounce border-2 border-yellow-200 hover:bg-white hover:scale-110 transition-all">
                  ...
                  {step === "envelope" && (
                     <div className="absolute -bottom-10 sm:-bottom-12 left-1/2 -translate-x-1/2 z-30 whitespace-nowrap">
        ...
        {step === "website" && (
          <motion.div
            key="website-view"
            className="relative z-10 min-h-screen py-6 md:py-10 px-4 md:px-8 xl:px-16 flex flex-col items-center"
          ...
          >
            ...
            <div className="glass-panel w-full max-w-5xl rounded-[30px] md:rounded-[40px] p-6 md:p-16 flex flex-col items-center text-center shadow-2xl mt-4 md:mt-10">
              ...
              <motion.div 
                ...
                className="relative space-y-6 w-full flex flex-col items-center pt-6 pb-2 md:pt-8 md:pb-4"
              >
                  {/* Floating Gold Hearts Animation (Responsive positioning) */}
                  <div className="absolute inset-0 pointer-events-none overflow-hidden z-0">
                     <motion.div animate={{ y: [0, -20, 0], scale: [1, 1.2, 1], opacity: [0.3, 0.8, 0.3] }} transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }} className="absolute top-2 left-4 md:top-10 md:left-[25%] text-yellow-500 drop-shadow-md">
                        <Heart className="w-5 h-5 md:w-6 md:h-6" fill="currentColor" strokeWidth={0} />
                     </motion.div>
                     <motion.div animate={{ y: [0, -30, 0], scale: [0.8, 1.1, 0.8], opacity: [0.2, 0.6, 0.2] }} transition={{ duration: 5, repeat: Infinity, delay: 1, ease: "easeInOut" }} className="absolute top-0 right-4 md:top-4 md:right-[25%] text-yellow-600 drop-shadow-md">
                        <Heart className="w-6 h-6 md:w-8 md:h-8" fill="currentColor" strokeWidth={0} />
                     </motion.div>
                     <motion.div animate={{ y: [0, -15, 0], scale: [1, 1.3, 1], opacity: [0.4, 0.9, 0.4] }} transition={{ duration: 3.5, repeat: Infinity, delay: 0.5, ease: "easeInOut" }} className="absolute bottom-6 left-8 md:bottom-10 md:left-[20%] text-yellow-400 drop-shadow-md">
                        <Heart className="w-4 h-4 md:w-5 md:h-5" fill="currentColor" strokeWidth={0} />
                     </motion.div>
                     <motion.div animate={{ y: [0, -25, 0], scale: [0.9, 1.2, 0.9], opacity: [0.3, 0.7, 0.3] }} transition={{ duration: 4.5, repeat: Infinity, delay: 1.5, ease: "easeInOut" }} className="absolute bottom-12 right-8 md:bottom-20 md:right-[20%] text-yellow-500 drop-shadow-md">
                        <Heart className="w-5 h-5 md:w-6 md:h-6" fill="currentColor" strokeWidth={0} />
                     </motion.div>
                  </div>

                  {/* Names perfectly stacked on mobile, inline on desktop */}
                  <div className="relative z-10 flex flex-col items-center">
                    ...
                    <div className="mt-8 md:mt-10 flex flex-col items-center space-y-3 md:space-y-4">
                       <p className="font-lato text-xs sm:text-sm md:text-base tracking-[0.25em] md:tracking-[0.4em] text-gray-500 uppercase font-bold text-center px-4">
                          Are Getting Married
                       </p>
                       <div className="w-12 md:w-16 h-[2px] bg-yellow-600/50 rounded-full"></div>
                       <p className="font-playfair text-xl sm:text-2xl md:text-3xl text-gray-800 font-medium tracking-widest md:tracking-wider">
                          {event_info.date}
                       </p>
                    </div>
                  </div>
              ...
              <p className="mt-10 md:mt-12 text-lg sm:text-xl md:text-2xl text-gray-700 font-light italic leading-relaxed max-w-2xl mx-auto px-2">
              ...
              <div className="w-full max-w-4xl px-2 md:px-0">
                  <motion.h2 
                    variants={fadeInUp} initial="hidden" whileInView="visible" viewport={{ once: true }}
                    className="font-playfair text-4xl md:text-5xl text-gray-900 font-extrabold mb-12 md:mb-16"
                  >
                    Event Schedule
                  </motion.h2>
              ...
              >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-10 items-center">
                    <div className="flex flex-col items-center md:items-start text-center md:text-left space-y-4 px-2">
                      <div className="p-3 md:p-4 bg-yellow-50 rounded-full text-yellow-600 mb-2">
                        <MapPin className="w-8 h-8 md:w-10 md:h-10" />
                      </div>
                      <p className="font-playfair text-3xl md:text-4xl font-bold text-gray-900 leading-tight">
                        @ {location.venue} <br/>
                        <span className="text-xl md:text-2xl font-normal text-gray-700">{location.city}, {location.country}</span>
                      </p>
                      <p className="text-gray-600 font-lato text-sm md:text-base leading-relaxed max-w-sm">
              ...
              <motion.div 
                variants={fadeInUp} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }}
                className="w-full max-w-2xl bg-white/50 border border-white rounded-2xl md:rounded-3xl p-6 sm:p-8 md:p-12 shadow-sm"
              >
                  ...
                  <p className="text-gray-700 font-light text-sm md:text-base mb-8 md:mb-10">
                    Your presence means the world to us. Please kindly let us know if you will be able to join our celebration.
                  </p>

                  <form className="space-y-5 md:space-y-6 text-left">
                    <div>
                      <label className="block text-gray-700 font-medium text-sm md:text-base mb-2 pl-1">Full Name</label>
                      <input type="text" className="w-full bg-white/80 border border-gray-300 rounded-lg md:rounded-xl px-4 py-3 text-sm md:text-base focus:outline-none focus:ring-2 focus:ring-yellow-500" placeholder="John & Jane Doe" />
                    </div>

                    <div>
                      <label className="block text-gray-700 font-medium text-sm md:text-base mb-2 pl-1">Number of Guests</label>
                      <div className="relative">
                        <select className="appearance-none w-full bg-white/80 border border-gray-300 rounded-lg md:rounded-xl px-4 py-3 text-sm md:text-base text-gray-700 focus:outline-none focus:ring-2 focus:ring-yellow-500">
                    ...
                    <div>
                      ...
                      <textarea className="w-full bg-white/80 border border-gray-300 rounded-lg md:rounded-xl px-4 py-3 text-sm md:text-base focus:outline-none focus:ring-2 focus:ring-yellow-500 h-24" placeholder="Any allergies or preferences..."></textarea>
                    </div>

                    <button type="button" className="w-full bg-gradient-to-r from-yellow-600 to-yellow-700 text-white font-playfair font-bold text-lg md:text-xl py-3 md:py-4 rounded-lg md:rounded-xl hover:shadow-lg hover:from-yellow-700 hover:to-yellow-800 transition-all">
                      Send RSVP
                    </button>
                  </form>
              </motion.div>
              ...
              <motion.div 
                ...
                className="w-full mt-16 md:mt-24 pt-8 md:pt-10 border-t border-yellow-600/30 flex flex-col items-center"
              >
                  {/* Marquee Text */}
                  <div className="marquee-container mb-10 md:mb-12">
                      <div className="marquee-text font-playfair text-xl md:text-3xl text-yellow-700 tracking-[0.3em] md:tracking-[0.4em] font-bold uppercase">
                  ...
                  <div className="flex flex-col md:flex-row justify-center items-center gap-4 md:gap-8 text-gray-800 text-sm md:text-base font-medium mb-12 md:mb-16">
                    <div className="flex items-center gap-2">
                      <Phone className="w-4 h-4 md:w-5 md:h-5 text-yellow-600" /> {contact_details.phone_number} ({contact_details.contact_person})
                    </div>
                    <div className="hidden md:block w-2 h-2 rounded-full bg-yellow-600"></div>
                    <div className="flex items-center gap-2">
                      <Mail className="w-4 h-4 md:w-5 md:h-5 text-yellow-600" /> {contact_details.email_address}
                    </div>
                  </div>

                  {/* Simple, Professional Bottom Design */}
                  <div className="flex flex-col items-center justify-center text-center pb-6 md:pb-8">
                     <h2 className="font-playfair text-3xl md:text-5xl text-gray-900 italic font-medium mb-3 md:mb-4">
                         {couple.bride_name} & {couple.groom_name}
                     </h2>
                     <p className="font-lato text-[10px] md:text-xs tracking-[0.2em] md:tracking-[0.3em] text-gray-500 uppercase mb-2 md:mb-3">
                         Are Getting Married
                     </p>
                     <p className="font-lato text-xs md:text-sm tracking-[0.15em] md:tracking-[0.2em] text-gray-800 uppercase font-medium">
